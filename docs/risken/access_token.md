...
## アクセストークンを発行する（Organization）

Organizationでもアクセストークンを発行できます。操作方法はプロジェクトの場合と同じです（詳細は [アクセストークンを発行する](#アクセストークンを発行する) を参照してください）。プロジェクトと同様に、トークン管理者が存在することをご確認ください。

???+ warning
    なお、Organizationのアクセストークンについては、[アクセストークンの有効期限切れを検知する](#アクセストークンの有効期限切れを検知する)で説明している有効期限切れの検知機能は提供されていません。
