...
Organization Modeでは、Organizationに所属する全プロジェクトのアラートを一括で通知する **Organization通知** を設定できます。
プロジェクトレベルのアラートが発生した際、そのプロジェクトが所属するOrganizationに通知先が設定されていれば、自動的にOrganization通知も送信されます。

???+ info "プロジェクトアラートとの違い"
    プロジェクトアラートはルール・条件・通知の3要素で構成されますが、Organization通知は **現在のところ通知先の設定のみ** をサポートしています
